...
static int lua_soundmeta_tostring(lua_State* L)
{
	...
	milk::sound_data* soundData = milk::audio_get_sounddata(sound->uid);
...
static int lua_musicmeta_tostring(lua_State* L)
{
	...
	milk::music_data* musicData = milk::audio_get_musicdata(music->uid);
...
static int lua_audio_new_music(lua_State* L)
{
	...
	if (uid != milk::INVALID_UID) {
		...
		music->uid = uid;
		music->state = milk::sample_state::STOPPED;
		music->volume = 1.0f;
...
static const milk::luaM_Enum lua_audio_states_enum[] = {
	{ "PLAYING", (int)milk::sample_state::PLAYING },
	{ "PAUSED", (int)milk::sample_state::PAUSED },
	{ "STOPPED", (int)milk::sample_state::STOPPED }
};
