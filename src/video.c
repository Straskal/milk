...
void drawPixel(Video *video, int x, int y, uint32_t color) {
    if (video->clipRect.left <= x && x < video->clipRect.right && video->clipRect.top <= y &&
        y < video->clipRect.bottom)
...
static void
__drawBuffer(Video *video, uint32_t *buffer, int x, int y, int w, int h, int pitch, float scale, uint8_t flip,
             uint32_t color) {
    if (scale <= 0)
        return;
...
void drawSprite(Video *video, Bitmap *bmp, int index, int x, int y, int w, int h, float scale, uint8_t flip,
                uint32_t color) {
    ...
    __drawBuffer(video, BUFFER_CHUNK(bmp, row, column), x, y, w * SPRITE_SIZE, h * SPRITE_SIZE, bmp->width, scale, flip,
                 color);
}
...
void drawFont(Video *video, Bitmap *bmp, int x, int y, const char *text, int scale, uint32_t color) {
    ...
    Bitmap bitmap = bmp != NULL ? *bmp : (Bitmap) {
    ...
    while ((curr = *text)) {
        ...
        switch (curr) {
            ...
            default: {
                ...
                __drawBuffer(video, BUFFER_CHUNK(&bitmap, row, col), xCurrent, yCurrent, SPRITE_SIZE, SPRITE_SIZE,
                             bitmap.width, scale, 0, color);
