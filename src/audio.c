...
void stopSound(Audio *audio, int slotId)
{
    ...
    for (int i = 0; i < MAX_SOUND_SLOTS; i++) {
        if ((slotId == ALL_SLOTS || slotId == i) && slots[i].state == PLAYING) {
...
void resumeSound(Audio *audio, int slotId)
{
    ...
    SoundSlot *slots = audio->soundSlots;
    for (int i = 0; i < MAX_SOUND_SLOTS; i++) {
        if ((slotId == ALL_SLOTS || slotId == i) && slots[i].state == PAUSED)
...
{
    // 3 - 1 (mono) = 2 iterations per sample. This is because we the destination is stereo.
    // We duplicate the mono samples in both the L and R channels.
    // 3 - 2 (stereo) = 1 iterations per sample. Stereo is already interleaved.
    const int iterationsPerSample = 3 - numChannels;

    int16_t sourceSample, destSample;
    while (numSamples--) {
        // Take the source sample and apply volume effects.
        sourceSample = (*source++ * volume) / MAX_VOLUME;
        for (int i = 0; i < iterationsPerSample; i++) {
            // We clamp each sample every time we mix.
            // Clamping should really only be applied once the entire audio is mixed.
            // What we're doing here can cause clipping.
...
void mixSamplesIntoStream(Audio *audio, int16_t *stream, int numSamples)
{
    ...
    if (audio->streamSlot.state == PLAYING) {
        ...
        mixSamples(
            ...
            streamData->channelCount,
            audio->streamSlot.volume);

        if (finished)
            audio->streamSlot.state = STOPPED;
    }

    // Mix individual slots.
    SoundSlot *slots = audio->soundSlots;
    for (int i = 0; i < MAX_SOUND_SLOTS; i++) {
        if (slots[i].state == PLAYING) {
            if (slots[i].remainingSamples > 0) {
                ...
                mixSamples(
                    ...
                    slots[i].position,
                    samplesToMix,
                    slots[i].soundData->channelCount,
                    slots[i].volume);
